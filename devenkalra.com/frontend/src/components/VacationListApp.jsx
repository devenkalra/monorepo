...
export function VacationListApp() {
  ...
  const loadArchivedLists = useCallback(async () => {
    ...
    setArchivedLists(Array.isArray(data) ? data : data.results || []);
  }, [token]);
  ...
  const loadCatalog = useCallback(async () => {
    ...
    const data = await api(`items/?${params}`, { token });
    setCatalog(Array.isArray(data) ? data : data.results || []);
  }, [token, catalogQ, catalogCategory, catalogTags]);
  ...
  }, [isAuthenticated, token, loadLists, loadCategories, loadTags]);
