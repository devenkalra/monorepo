...
export default function ProgressModal({ taskId, taskType, onComplete, onCancel }) {
  ...
  useEffect(() => {
    ...
    const pollProgress = async () => {
      try {
        const response = await api.fetch(`/api/entities/tasks/${taskId}/progress/`);
  ...
  const handleCancelConfirm = async () => {
    ...
    try {
      await api.fetch(`/api/entities/tasks/${taskId}/cancel/`, {
        method: 'POST'
      });
    } catch (error) {
