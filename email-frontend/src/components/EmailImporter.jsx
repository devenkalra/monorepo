...
function EmailImporter() {
  ...
  const fetchConfigs = async () => {
    try {
      ...
      if (!resp.ok) {
        console.error('Failed to fetch configs:', resp.status, resp.statusText);
        return;
      }
      const data = await resp.json();
      // Handle paginated response
      const configsList = data.results || data;
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-500 space-y-1">
                        ...
                        {config.to_date && <p>To: {config.to_date}</p>}
                ...
                <form onSubmit={handleSaveConfig} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={configForm.to_date}
                        onChange={(e) => setConfigForm(prev => ({ ...prev, to_date: e.target.value }))}
                        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-transparent text-gray-900 dark:text-gray-100"
                      />
                    </div>
                  </div>
