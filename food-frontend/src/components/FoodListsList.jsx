...
export default function FoodListsList({ apiBase }) {
  ...
  const loadLists = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.fetch(`${apiBase}/food-lists/`);
      const data = await res.json();
      setLists(Array.isArray(data) ? data : data.results || []);
    } catch (err) {
      console.error('Failed to load food lists', err);
      setLists([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (e, listId) => {
    ...
    try {
      await api.fetch(`${apiBase}/food-lists/${listId}/`, { method: 'DELETE' });
      loadLists();
    } catch (err) {
  ...
  return (
    <div>
      <div className="mb-4 flex justify-between items-center">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Food Lists</h2>
        <Link
          to="/food-lists/create"
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {list.foods?.length || 0} foods
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <Link
                      to={`/food-lists/${list.id}/edit`}
