...
passphrases = [
    ...
    'xxx' # from the mock user input instruction
]
salts = [
    b"bldrdojo-salt-1",
...
for entity in Entity.objects.filter(is_encrypted=True):
    ...
    for salt in salts:
        for pw in passphrases:
            try:
                # Key derivation: PBKDF2-HMAC-SHA256, 600,000 iterations, 32 bytes key length
                key = hashlib.pbkdf2_hmac('sha256', pw.encode('utf-8'), salt, 600000, 32)
                aesgcm = AESGCM(key)
                plaintext_bytes = aesgcm.decrypt(iv, ciphertext, None)
                plaintext = plaintext_bytes.decode('utf-8')
                print(f"Decrypted! key='{pw}', salt='{salt.decode()}':")
