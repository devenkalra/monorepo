...
from libs.py_cli.py_cli.shell import taskInterface
...
def generate_id(full_file_path, mount_path, volume_tag):
    ...
    return (hashlib.md5(f"{volume_tag}/{relative_path_local}::{content_hash}".encode('utf-8')).hexdigest(), content_hash)
...
def create_document(filepath, file, scan_dir, volume_tag, skip_on, current_dir_files):
    ...
    if is_file_an_image_magic(filepath):
        ...
        # Add specific EXIF fields you want to search heavily
            ...
            doc['thumbnail'] = doc['thumbnail'].split("base64,")[1]
            doc['thumbnail'] = base64.b64decode(doc['thumbnail'])
            doc['thumbnail'] = io.BytesIO(doc['thumbnail'])
            doc['thumbnail'].seek(0)
...
def scan_and_upload(pmts):
    ...
    # ✅ NOTE: Move these settings calls OUTSIDE scanning in production (run once at startup/migration)
    ...
    # client.index(FOLDERS_INDEX).update_filterable_attributes([
    #     "parent_dir",
    #     "volume",
    #     "dir",
    #     "depth",
    #     "name",
    # ])
    # client.index(FOLDERS_INDEX).update_sortable_attributes([
    #     "name",
    #     "file_count",
    #     "child_count",
    #     "depth",
    ...
    for root, dirs, files in os.walk(start_dir):
        ...
        for file in files:
            ...
            if doc is not None:
                files = {"thumbnail": (doc["name"], doc["thumbnail"], 'image/jpeg')}
                del doc["thumbnail"]
                doc["meta"] = json.dumps(doc["exif"])
                del doc["exif"]

                response = requests.post(api_url + "catalog/ingest/", data=doc, files=files)
    ...
    for d in seen_folders:
        ...
        response = requests.post(api_url + "folders/", data=folder)
        if response.status_code != 201:
            print("Error")


    # Ensure root folder doc exists
    # (note_folder already adds "/", but we ensure parent_dir is consistent)
    # Upload in batches
    for i in range(0, len(folder_docs), FOLDER_BATCH_SIZE):
        batch = folder_docs[i:i + FOLDER_BATCH_SIZE]

        task = client.index(FOLDERS_INDEX).add_documents(batch)
        task_check_count = 0
        while True:
            ...
            if task_status.status == "failed":
                print("Add Folders Failed.")
                print("Errors: " + task_status.error["message"])
                break
            task_check_count += 1
            if task_check_count >= 30:
                print("Add Folders Failed, time out")
                break

            time.sleep(1)

        results = client.index(FOLDERS_INDEX).search('')
        print(f"Uploaded folder batch {i + len(batch)} / {len(folder_docs)}")
...
if __name__ == "__main__":
    ...
    print("\n--- Parsed Arguments ---")
    print(f"Scan Dirs:   {args.scan_dirs}")
    print(f"Volume Tags: {args.volume_tags}")
    print(f"Exclude:     {args.exclude}")
    print(f"Include:     {args.include}")

    # Example of how you would pair them up:
    scan_pairs = list(zip(args.scan_dirs, args.volume_tags))
    print(f"Scan Pairs:  {scan_pairs}")

    scan_and_upload(args)