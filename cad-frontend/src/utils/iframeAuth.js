/**
 * When CAD app runs in an iframe, it may not have access to the parent's localStorage
 * (e.g. due to storage partitioning). Request auth tokens from the parent.
 */
...
export function initIframeAuth() {
  ...
  // Retry request in case parent wasn't ready yet (max 10 retries)
...
/** Promise that resolves when we have a token (or give up). Use to delay API calls. */
