        ...
        <>
          ...
          <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-[#161b22] rounded-lg shadow-lg z-20 border border-gray-200 dark:border-[#30363d]">
            <div className="p-4 border-b border-gray-200 dark:border-[#30363d]">
              <p className="text-sm font-semibold text-gray-900 dark:text-[#e6edf3]">{user?.displayname || user?.username || 'User'}</p>
              <p className="text-xs text-gray-500 dark:text-[#8b949e] truncate">{user?.email}</p>
            </div>
            <div className="py-1">
              <button
                onClick={() => { setShowMenu(false); navigate('/profile'); }}
                className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-[#e6edf3] hover:bg-gray-100 dark:hover:bg-[#21262d] transition-colors flex items-center space-x-2"
              >
